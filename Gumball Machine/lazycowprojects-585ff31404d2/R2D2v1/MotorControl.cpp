...
void MotorControl::setUsesSingleMotor(boolean value) {
  this->_usesSingleMotor = value;
}
boolean   MotorControl::UsesSingleMotor(){
  return this->_usesSingleMotor;
}
...
void MotorControl::pushValues(){
  if (!this->_usesSingleMotor) {
    ...
    analogWrite(this->_pwmL, map(abs(leftWheel),0,MOTORCONTROL_RANGE,0,255));        
    analogWrite(this->_pwmR, map(abs(rightWheel),0,MOTORCONTROL_RANGE,0,255));
    digitalWrite(this->_dirL, (leftWheel >= 0)?LOW:HIGH);  
    digitalWrite(this->_dirR, (rightWheel <= 0)?LOW:HIGH);  
  }
  else {
    analogWrite(this->_pwmL, map(abs(this->_speed),0,MOTORCONTROL_MOTO_RANGE,0,255));        
    digitalWrite(this->_dirL, (this->_speed >= 0)?LOW:HIGH);  
    
    analogWrite(this->_pwmR, map(abs(this->_direction),0,MOTORCONTROL_RANGE,0,255));        
    digitalWrite(this->_dirR, (this->_direction < 0)?LOW:HIGH);     
  }
}
